...
|---|---|
| 作成日 | 2026-04-28 |
| 改訂日 | 2026-04-28 r3 (Step 2-2 Option A → B 改訂、FK SET NULL 採用) |
| ステータス | **改訂版 r3** — Step 2-2 Option B (PG 15+ partial SET NULL) 採用 + B-3b 完走 |
...
- Step 7 user 立ち会い動作確認 (PM/loans CRUD + reorder + 既存 expenses 表示)
- 段階 2 ALTER 直前の **orphan_count = 0 確認 SQL**
- ロールバック手順 A / A2 / B / C
- `legacy_key` 列による debug/rollback 識別性

### 2026-04-28 r2 (同日内、効率化改訂)

**改訂内容**: 全フェーズ今日完走モードに合わせ、待機ゼロ運用を徹底するための効率化を反映:

- 立ち会い動作確認は **user 自身が本部役 兼 動作確認実施者** として実施 (timing 依存解消)
- Step 4 commit 構造を簡素化: B-3b は callsite 7 個で B-3a の 10 個より少ないため、**3 commit** で完結 (phase 1 切替 + shim / phase 2 置換 + reorder / phase 3 shim 削除)
- Step 7 動作確認は機能必須 9 項目に絞り、技術確認系 (Console ログ / フラグ / 件数) は **optional** に格下げ
- Step 9 後始末の `docs/day_-summary.md` 追記は **optional** (本日完走完了記録は別 file または commit log で代替)
- 並列化の機会セクションを新設 (Step 2-1 / Step 4 のオーバーラップ)

**事業判断としての改訂理由**:

- 本日中に B-3b → Phase C (本部閲覧) → Phase D (代理編集 + Realtime) を一気通貫
- 各 Step の待機時間 (人作業や file 単位の sync) を最小化
- 安全要素 (orphan_count / rollback / 立ち会い) は維持、時間ベースゲートのみ撤廃

### 2026-04-28 r3 (同日内、Step 2-2 Option A → B 改訂)

**改訂内容**: Step 2-2 の FK カスケード戦略を **Option A (NO ACTION)** から **Option B (PG 15+ partial SET NULL)** に変更。

**Option B 採用理由**:
- 旧 Option A の前提「アプリ側 PM 削除 confirm dialog」が **未実装**で、本日完走スコープを膨らませる
- PG 15 から `ON DELETE SET NULL (column)` 構文で FK の特定列のみ NULL 化可能 (本番 PostgreSQL 17.6 で要件クリア)
- `useExpenses` の `payment_method ?? 'cash'` フォールバックと完全整合 (PM 削除時は UI 上「現金」表示)
- soft delete 設計 (expenses 物理削除しない) と整合
- アプリ側の追加実装ゼロ、UX 改善
- 設計論点 A 当時 C 案 (単純 SET NULL) は「複合 FK で client_id も NULL 化される」制約で却下したが、partial SET NULL なら **payment_method 列のみ NULL 化** で克服可能

**実施経緯 (孤児 28 件発覚 → 救済 cleanup)**:

事前 orphan_count = 28 検出。Check 4 (孤児詳細) 調査で内訳判明:

| client | payment_method | 件数 | 判定 |
|---|---|---|---|
| <client_A> (`<client_A>-...`) | `cash` | 23 | **実顧客データ** (B-3b migrate 未実行のため payment_methods に cash 行なし) |
| 自分 + <client_A> | `pm_<timestamp>` × 4 種 | 5 | smoke test / 過去テスト残骸 |

cleanup 手順 (シナリオ Y 採用):
1. **B-1**: <client_A>用 `payment_methods` に `cash` 行を手動 INSERT (legacy_key='cash')
2. **B-2**: 残り pm_xxx 系 5 件を `UPDATE ... SET payment_method = NULL`
3. **B-3**: orphan_count 再確認 → 0
4. **008 ALTER apply**: SET NULL 採用版で Success
5. **Step 8 動作確認**: 全 6 項目 PASS

**保持される安全策**:

- 事前 orphan_count = 0 確認 SQL は引き続き必須 (Option B でも FK 違反は同条件で発生)
- `cfo_paymentsLoansMigrated` フラグによる migrate 冪等性
- ロールバック手順 A / A2 / B / C
- `legacy_key` 列による debug/rollback 識別性
...
| **7** | 本番動作確認 | user 立ち会い、PM/loans CRUD + 並び替え動作確認 | 20 分 | 中 |
...
| **B. 複合 FK + ON DELETE NO ACTION (RESTRICT)** | PM を参照する expenses があると削除拒否 | ❌ (旧推奨、改訂 r3 で採用見送り) | アプリ側 confirm dialog の前提が未実装で本日完走スコープ膨張 |
| **C. 複合 FK + ON DELETE SET NULL (全列)** | 旧 PG 14 までの構文、両列 NULL 化 | ❌ | client_id は NOT NULL → SET NULL は失敗 |
...
| **F. 複合 FK + ON DELETE SET NULL (payment_method 列のみ、PG 15+)** | partial SET NULL: payment_method のみ NULL 化、client_id は維持 | ✅ **採用 (改訂 r3)** | PG 15+ の partial SET NULL 構文で C 案の制約を克服。useExpenses fallback と整合、アプリ側追加実装ゼロ、UX 改善 |

**採用 (改訂 r3)**: **案 F (Partial SET NULL)**。`ON DELETE SET NULL (payment_method)` 構文で FK の特定列のみ NULL 化、client_id は維持される。本番は PostgreSQL 17.6 で要件クリア。

**動作**:

1. PM 削除時、参照中 expenses の `payment_method` 列のみ NULL に flip
2. `client_id` / `deleted_at` / `amount` 等は維持 (expenses 行は残る)
3. `useExpenses` の `?? 'cash'` フォールバックで UI 表示は「現金」になる
4. アプリ側に追加の confirm dialog や事前処理は不要

**フォールバック**: `useExpenses` 既存の `payment_method ?? 'cash'` 処理は維持 → NULL flip 後の UI 表示が確実に「現金」になる責務を担う。
...
commit 段階構造は B-3b では **3 commit に簡略化** (改訂 r2):

1. **phase 1**: hook 切替 (`useLocalStorage` → `usePaymentMethods` / `useLoans`) + shim 設置
2. **phase 2**: 全 callsite (7 件) を hook 直呼び化 + reorder ロジック実装
3. **phase 3**: shim 削除

B-3a の 4 段階 (phase 1 / 2a / 2b / 3) より圧縮、callsite 数の少なさと配列 CRUD のシンプルさを根拠とする。

---

## 並列化の機会 (今日完走モードでの実時間短縮)

各 Step の依存関係を分析し、user / Claude のリソースをオーバーラップさせる:

### 並列ペア 1: Step 2-1 ↔ Step 4 初期ファイル作成

- Step 2-1 (Dashboard SQL Editor で DDL 実行) は **user の手作業 15 分**
- Step 4 の **API/hook ファイル骨格作成** (DDL に依存しない、スキーマ定義のみ参照) は Claude の Edit/Write 作業
- → 並列実行可: user が SQL を流す間、Claude は 4 ファイル (`paymentMethods.js` / `loans.js` / `usePaymentMethods.js` / `useLoans.js`) の骨格を並行生成
- 期待短縮: 15 分

### 並列ペア 2: Step 4 内の 4 ファイル同時 Write

- 上記 4 ファイルは相互独立 (hooks が API import するだけで cyclic dependency なし)
- → 単一レスポンスで Claude が 4 つの Write tool 呼び出しを並列実行可
- 期待短縮: 5 〜 10 分

### Step 5 はシリアル

- migrate ブロックの実装は Step 4 完了後 (hook が動く前提)
- 並列化不可

### Step 7 → Step 2-2 → Step 8 はシリアル

- 動作確認 → FK ALTER → FK 動作確認 は依存連鎖
- 並列化不可だが、各 Step 自体が短い (合計 55 分)

### 合計効果

- 改訂前 (24h 観察あり): 約 26 時間 (24h + 2 時間)
- 改訂 r1 (即時リリース): 約 2 〜 3 時間
- 改訂 r2 (今日完走、並列化): 約 **1.5 〜 2 時間** (並列ペア 1 で 15 分、並列ペア 2 で 5-10 分の短縮)
...
### 4-2. App.jsx 切替 (3 commit 構造、改訂 r2)

| commit | 内容 |
|---|---|
| **phase 1** | L262 / L301 の useLocalStorage を usePaymentMethods / useLoans に置換 + setPaymentMethods / setLoans shim 設置 (動作互換) |
| **phase 2** | 7 callsite (PM 4 + loans 3) を hook 直呼びに一括置換 + reorder ロジック実装 |
| **phase 3** | shim 削除、useCallback import 整理 |

### 4-3. 動作確認

phase ごとに build check + 動作確認 → commit。B-3a の Phase 2a/2b/3 を 1 commit に統合した形 (callsite 数が少ないため安全に圧縮可)。
...
## Step 7. 本番動作確認 (20 分、user 立ち会い)

立ち会い実施者は **user 自身 (本部役 兼 動作確認実施者)**。B-3a 動作確認も user 自身が実施した前例を踏襲、<client_A>への依頼による timing 待ちを発生させない。

### 必須 9 項目 (機能確認)

- [ ] PM 新規作成 (create) → 一覧追加 → DB 反映確認
- [ ] PM 編集 (update: label / color / closingDay 等) → 反映確認
- [ ] PM 削除 (delete) → confirm dialog → 'cash' 振替 or キャンセル動作
- [ ] **PM drag-drop reorder** → 並び替え反映 → reload 後も順序保持
- [ ] loans 新規作成 (create) → 一覧追加
- [ ] loans 編集 (update) → 反映
- [ ] loans 削除 (delete) → 一覧から消失
- [ ] 既存 expenses が削除済みでない PM を参照していることを確認 (孤児ゼロ)
- [ ] 既存 expenses の PM バッジが正しく表示される

### Optional 項目 (技術確認、skip 可)

- Console に `[migrate] payments/loans` ログ
- `localStorage.cfo_paymentsLoansMigrated === "1"` 確認
- Dashboard で payment_methods / loans 行件数 = localStorage 件数

> **改訂 r2**: 旧チェックリスト 13 項目を機能必須 9 + optional 3 に再分類。技術確認系は本日完走モードでは時短のため skip 可。
...
## Step 8. 段階 2 動作確認 (15 分、改訂 r3 で Option B シナリオに更新)

Option B (partial SET NULL) 採用後の検証シナリオ:

- [ ] 「現金」で支出 1 件記録 (例: ランチ 1000 円) → expenses に payment_method='cash' で行追加
- [ ] 決済手段画面で「現金」を削除 → 削除成功 (FK 違反は出ない、NULL flip が走る)
- [ ] 支出履歴で当該 expense を確認 → UI は「現金」表示 (`?? 'cash'` fallback)、DB は payment_method=NULL
- [ ] Supabase で `select payment_method from expenses where id=...` を実行 → NULL を確認
- [ ] リロード → 上記すべて永続化
- [ ] 'cash' を再作成 → 同 expense は引き続き「現金」表示 (NULL のまま、fallback 値が偶然一致)

---

## Step 9. 後始末 (5 分、改訂 r2 で短縮)

### 必須

- [ ] user の localStorage に `cfo_paymentMethods` / `cfo_loans` が残っていることを確認 (削除しない方針)
- [ ] `docs/phase-b-3b-plan.md` 末尾の「実施結果ログ」を記入

### Optional (skip 可)

- `docs/todo-followups.md` の 3 項目を再評価 (item 3 auth timeout が本フェーズで顕在化したか)
- `docs/day_-summary.md` (or 同等) に B-3b 完了を追記 → **本日完走モードでは commit log で代替可**
...
2. クライアント側 `cfo_paymentsLoansMigrated` フラグを hot-fix で 1 回だけ削除する版を出すか、<client_A>に DevTools から手動削除依頼
...
| R3 | App.jsx の差替で UI リグレッション (PM 一覧が空、loans が消失等) | **中** | Step 7 で user 立ち会い動作確認、必須 9 項目を漏らさず実施 |
...
| 項目 | 値 |
|---|---|
| 開始時刻 | 2026-04-28 13:00 JST 頃 |
| 終了時刻 | 2026-04-28 14:00 JST 頃 |
| 実施者 | 佐藤 |
| Step 2-1 SQL 実行結果 | Success (13 → 15 tables、payment_methods + loans 追加) |
| Step 3 DB 確認結果 | テーブル / index / RLS / Realtime publication すべて期待通り |
| Step 7 立ち会い実施時刻 | 2026-04-28 14:00 JST 頃 (本番 deploy 直後、SW cache 起因で migrate 初回不発 → hard reload で再実行成功、9 必須項目すべて PASS) |
| 本番 payment_methods 行数 | 2 件 (<client_self> / <client_A> 各 cash 1 件、smoke test 後 cleanup 済) |
| 本番 loans 行数 | 0 件 (移行前 localStorage に loans データなし) |
| 不具合・特記事項 | PWA Service Worker キャッシュで deploy 直後の旧バンドル serve が発生 → Cmd+Shift+R で解消。todo-followups.md #4 として正式記録 |

### 段階 2 (Step 7 直後に即時実施、改訂 r3 で Option B 採用)
| 項目 | 値 |
|---|---|
| 段階 2 開始時刻 | 2026-04-28 14:30 JST 頃 |
| 段階 2 終了時刻 | 2026-04-28 15:00 JST 頃 |
| 事前確認 orphan_count | 28 → cleanup 後 0 (詳細は改訂履歴 r3 参照) |
| Step 2-2 ALTER 実行結果 | Success (Option B partial SET NULL 採用、`SET NULL (payment_method)` 構文) |
| Step 8 動作確認結果 | 全 6 項目 PASS (NULL flip + fallback 動作確認、リロード永続化) |
| 不具合・特記事項 | 当初 orphan 28 件発覚。<client_A> cash 23 件は実顧客データのため手動 seed (legacy_key='cash') で救済、残り 5 件 (pm_xxx 形式の test 残骸) は NULL 化。Option A (NO ACTION) → Option B (Partial SET NULL) へ plan 改訂 r3 適用 |
