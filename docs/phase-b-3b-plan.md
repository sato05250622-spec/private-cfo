...
| 想定所要時間 | **2.5 〜 3.5 時間** (B-3a 同等、callsite が少ない代わりに ALTER 段階リリース対応) |
| 着手前提 | B-3a が main にマージ済 (commit `bda5165`) かつ本番 24 時間運用観察を完了 |
...
- **既存テーブル `expenses` を変更する**(B-3b で初めて ALTER を実施)
  ...
  - ALTER は **段階 2** (全 client migrate 完了後) で適用、段階 1 では FK 無しで運用継続
...
| # | フェーズ | 作業 | 所要 | リスク |
|---|---|---|---|---|
| **1** | Pre-flight | B-3a 観察期間 24h クリア確認 / バックアップ最新化 | 5 分 | 低 |
| **2-1** | DB 適用 段階1 | `payment_methods` + `loans` の DDL 流し込み (FK 制約は次段階) | 15 分 | 中 |
...
| **8** | 観察期間 | 24h 観察し全 client が `cfo_paymentsLoansMigrated` 立っているか確認 | 24h | 低 |
| **2-2** | DB 適用 段階2 | `expenses.payment_method` に FK 制約追加 (ALTER) | 10 分 | **高** |
| **9** | 段階2 動作確認 | PM 削除時の expenses への影響を確認 | 15 分 | 中 |
| **10** | 後始末 | フラグ確認、ログ記入、`docs/todo-followups.md` 該当項目消し込み | 10 分 | 低 |

合計: 約 2.5 〜 3.5 時間 + 段階1〜2 間の **24 時間観察**
...
### 論点 E: 段階的リリース (FK 追加を後回しにする理由)

FK を即時追加すると、移行未完了 client の expenses が `payment_methods` 行に存在しない id を参照する状態が生じ、FK 違反で **テーブルそのものが INSERT 不能**になる。これを回避するため:

- **段階 1**: payment_methods / loans テーブルを作成、アプリを hook 駆動に切替、全 client が migrate 完了するまで FK 制約は **追加しない**
- **段階 2**: 24h 観察で全 client が `cfo_paymentsLoansMigrated === "1"` を確認後、`ALTER TABLE expenses ADD CONSTRAINT ...` で FK 追加
...
`supabase/migrations/008_phase_b3b_expenses_payment_fk.sql` (段階 2 用、24h 観察後):
...
| 段階リリース | 単発 | **2 段階** (FK 追加は別 migration) | 24h 観察期間が必要 |
...
- **判断**:
  - B-3b **着手前** に optional で調査 (timeout 値見直し or リトライ戦略)
  - もしくは並行タスクとして別 PR で対応
  - **本フェーズの blocker ではない** (機能は動作する) が、UX 改善のため早期対応推奨
  - B-3b Step 7 の動作確認時に同症状が顕著に出るかは要観察
...
- [ ] B-3a 本番運用が **24 時間以上経過** していることを確認 (<client_A> さんから「予算が消えた / 数字が違う」報告ゼロ)
- [ ] Dashboard で `legacy_key` が空の budgets/week_budgets/week_cat_budgets 行が増加していること (= 新規予算編集が DB に届いている証跡)
- [ ] 直近 1 時間以内に Supabase Dashboard の手動 snapshot を取得
- [ ] ローカル `git status` クリーン、`main` から作業ブランチ `feat/phase-b-3b-payments-loans` を切る (本ドラフトと同 branch を継続使用)
...
- [ ] <client_A> さんから「決済手段が消えた / 並び順が崩れた / 借入が表示されない」報告ゼロ
- [ ] Dashboard で legacy_key 空の payment_methods / loans 行が増加 (= 新規操作が DB に届いている)

---

## Step 2-2. DB 適用 段階 2 (10 分、観察期間後)
...
| 項目 | 値 |
|---|---|
| 観察期間開始 | `2026-__-__ __:__ JST` |
| 観察期間終了 | `2026-__-__ __:__ JST` |
| 事前確認 orphan_count | `___` (0 でなければ ALTER 中止) |
| Step 2-2 ALTER 実行結果 | `____________` |
| Step 9 動作確認結果 | `____________` |
| 不具合・特記事項 | `____________` |
