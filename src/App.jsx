...
// recharts はメインチャンクから分離するため直接 import しない。チャートは
// DashboardCharts / AssetSheetViewer を React.lazy で遅延ロードし、表示時に初めて取得する。
import {
...
// AssetSheetViewer は recharts (累計残高推移グラフ) を含むため遅延ロード。
const AssetSheetViewer = lazy(() => import("./components/AssetSheetViewer"));
...
// recharts 依存のダッシュボードチャートは遅延ロード (メインチャンク分離)。
// 同一モジュールを参照するため recharts は 1 チャンクを共有し、初回チャート表示時にのみ取得される。
const CategoryPieChart = lazy(() => import("./components/charts/DashboardCharts").then(m => ({ default: m.CategoryPieChart })));
const YearlyAreaChart  = lazy(() => import("./components/charts/DashboardCharts").then(m => ({ default: m.YearlyAreaChart })));

// チャート遅延ロード中のプレースホルダ (NAVY 背景に馴染む簡素なローディング表示)。
function ChartFallback({ height }) {
  return (
    <div style={{height, display:"flex", alignItems:"center", justifyContent:"center", color:TEXT_MUTED, fontSize:12}}>
      グラフを読み込み中…
    </div>
  );
}
            ...
            {catBreakdown.length>0&&(
              <div style={{background:CARD_BG,padding:"16px 18px 8px",marginBottom:1}}>
                <Suspense fallback={<ChartFallback height={240}/>}>
                  <CategoryPieChart catBreakdown={catBreakdown} catLabelLayout={catLabelLayout}/>
                </Suspense>
              </div>
            )}
            ...
            <div style={{background:CARD_BG,padding:"20px 20px 16px",marginBottom:1}}>
              ...
              <div style={{height:200}}>
                <Suspense fallback={<ChartFallback height={200}/>}>
                  <YearlyAreaChart yearlyData={yearlyData} today={today}/>
                </Suspense>
              </div>
            </div>
